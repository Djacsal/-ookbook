<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="description" content="Фото рецепт приготовления карасей в сметане в духовке. Самый, наверное, простой рецепт! 
    Нам просто необходимо почистить и выпотрошить рыбу, залить ее сметаной, тщательно перемешать, 
    уложить на нарезанный полукольцами лук и запечь в духовке до полного приготовления, а именно – 
    до запечённого состояния сметаны! Карась в сметане запеченный в духовке готов, подаем его и получаем удовольствие!">
    <link rel="stylesheet" href="/css/Dish.css?<?echo time();?>">
    <link rel="stylesheet" href="/css/Normalize.css">
    <title>Караси в сметане в духовке</title>
</head>
<body>
    <header>
        <div class="head">
            <div class="logo">
                <img src="/img/logo.png" alt="Логотип">
            </div>
            <div class="header_social_network"> 
                <div class="search_form">
                    <form action="/html/search/index.php" method="get">
                        <input type="text" id="dish_name" name="q" required placeholder="Искать здесь...">
                        <button type="submit"></button>
                    </form>
                </div>
                <div class="social_network">
                    <div class="vk, network">
                        <a href="https://vk.com/djacsal228" title="Перейти на стринуцу во Вконтакте" target="_blank"><img src="/img/VK blak.png" alt="Вконтакте"></a>
                    </div>
                    <div class="facebook, network">
                        <a href="https://www.facebook.com" title="Перейти на стринуцу в Фейсбук" target="_blank"><img src="/img/Facebook.png" alt="Фейсбук"></a>
                    </div>
                    <div class="instagram, network">
                        <a href="https://www.instagram.com/viniadukhovskii" title="Перейти на стринуцу в Инстаграмм" target="_blank"><img src="/img/instagram.png" alt="Инстаграмм"></a>
                    </div>
                </div>
            </div>
        </div>
        <nav>
            <ul class="navigation">
                <li><a class="nav_title" title="Перейти на главную страницу" href="/html/index.php">Главная</a></li>
                <li><a class="nav_title" title="Перейти на страницу с салатами" href="/html/Salads/Salads.php">Салаты</a></li>
                <li><a class="nav_title" title="Перейти на страницу с супами" href="/html/Soups/Soups.php">Супы</a></li>
                <li><a class="nav_title" title="Перейти на страницу с грибами и овощами" href="/html/Mushrooms_vegetables/Mushrooms_vegetables.php">Грибы и овощи</a></li>
                <li><a class="nav_title" title="Перейти на страницу с рыбой и морепродуктами" href="/html/Fish_seafood/Fish_seafood.php">Рыба и морепродукты</a></li>
                <li><a class="nav_title" title="Перейти на страницу с с рецептами из мяса" href="/html/Meat/Meat.php">Рецепты из мяса</a></li>
                <li><a class="nav_title" title="Перейти на страницу с выпечкой" href="/html/Bakery/Bakery.php">Выпечка</a></li>
                <li><a class="nav_title" title="Перейти на страницу с десертами" href="/html/Desserts/Desserts.php">Десерты</a></li>
            </ul>
        </nav>
    </header>
    <main>
        <div class="main_container">
            <div class="dish">
                <div class="dish_head dish_container">
                    <div class="dish_title">
                        <p class="dish_title">Караси в сметане в духовке</p>
                    </div>
                    <div class="dish_img">
                        <img class="img_width" src="/img/Fish_seafood_img/2.png" alt="Караси в сметане в духовке" width="750px">
                    </div>
                    <div class="dish_info">
                        Фото рецепт приготовления карасей в сметане в духовке. Самый, наверное, простой рецепт! 
                        Нам просто необходимо почистить и выпотрошить рыбу, залить ее сметаной, тщательно перемешать, 
                        уложить на нарезанный полукольцами лук и запечь в духовке до полного приготовления, а именно – 
                        до запечённого состояния сметаны! Карась в сметане запеченный в духовке готов, подаем его и получаем 
                        удовольствие!
                    </div>
                </div>
                <div class="dish_ingredients dish_container">
                    <div class="dish_title">
                        <p class="dish_title">Для этого блюда понадобится</p>
                    </div>
                    <div class="dish_img">
                        <img class="img_width" src="/img/Fish_seafood_img/2/1.jpg" alt="Ингридиенты" width="650px">
                    </div>
                    <div class="dish_info">
                        Караси – 5 штук;<br>
                        Сметана – 100г.;<br>
                        Репчатый лук – 1 головка;<br>
                        Сливочное мало – 60г.;<br>
                        Зелень укропа и петрушки;<br>
                        Соль и черный перец – по вкусу.
                    </div>
                </div>
                <div class="dish_recipe dish_container">
                    <div class="dish_title">
                        <p class="dish_title">Рецепт</p>
                    </div>
                    <div class="dish_info">
                        Кто бы что не говорил, но я очень люблю жареных карасей! Ну и ладно что они костлявые, зато вкусно, 
                        а самое главное – сам добыл, сам пожарил и съел! Отдельно стоит остановиться над такой разновидностью рецепта, 
                        как караси в сметане! В идеале жарить их конечно в русской печке, но если готовим дома – то и духовка очень 
                        даже ничего себе выручает! Итак, рыбу наловили, теперь будем запекать карасей в сметане!
                    </div>
                    <div class="dish_img">
                        <img class="img_width" src="/img/Fish_seafood_img/2/2.jpg" width="650px">
                    </div>
                    <div class="dish_info">
                        Для начала, выложим карасей на доску или плоское блюдо, немного ее посолим и поперчим со всех сторон, 
                        уберем в сторону мариноваться минут на 10.
                    </div>
                    <div class="dish_img">
                        <img class="img_width" src="/img/Fish_seafood_img/2/3.jpg" width="650px">
                    </div>
                    <div class="dish_info">
                        В сметану добавим нарубленную зелень укропа и петрушки, щепотку соли и немного черного перца.
                    </div>
                    <div class="dish_img">
                        <img class="img_width" src="/img/Fish_seafood_img/2/4.jpg" width="650px">
                    </div>
                    <div class="dish_info">
                        Хорошо перемешаем.
                    </div>
                    <div class="dish_img">
                        <img class="img_width" src="/img/Fish_seafood_img/2/5.jpg" width="650px">
                    </div>
                    <div class="dish_info">
                        Противень намажем мягким сливочным маслом.
                    </div>
                    <div class="dish_img">
                        <img class="img_width" src="/img/Fish_seafood_img/2/6.jpg" width="650px">
                    </div>
                    <div class="dish_info">
                        Лук нарежем тонкими полукольцами, выложим в противень.
                    </div>
                    <div class="dish_img">
                        <img class="img_width" src="/img/Fish_seafood_img/2/7.jpg" width="650px">
                    </div>
                    <div class="dish_info">
                        Промаринованных карасей переместим в глубокую миску.
                    </div>
                    <div class="dish_img">
                        <img class="img_width" src="/img/Fish_seafood_img/2/8.jpg" width="650px">
                    </div>
                    <div class="dish_info">
                        Зальем рыбу сметаной с зеленью.
                    </div>
                    <div class="dish_img">
                        <img class="img_width" src="/img/Fish_seafood_img/2/9.jpg" width="650px">
                    </div>
                    <div class="dish_info">
                        Затем выложим карасей в противень для запекания одним слоем.
                    </div>
                    <div class="dish_img">
                        <img class="img_width" src="/img/Fish_seafood_img/2/10.jpg" width="650px">
                    </div>
                    <div class="dish_info">
                        Отправим противень с карасями в предварительно разогретую до 200 градусов духовку на 30-35 минут.
                    </div>
                    <div class="dish_img">
                        <img class="img_width" src="/img/Fish_seafood_img/2/11.jpg" width="650px">
                    </div>
                    <div class="dish_info">
                        Достаём готовые караси в сметане из духовки.
                    </div>
                    <div class="dish_img">
                        <img class="img_width" src="/img/Fish_seafood_img/2/12.jpg" width="650px">
                    </div>
                    <div class="dish_info">
                        Получается вот такая красота как показано на этой фотографии.
                    </div>
                    <div class="dish_img">
                        <img class="img_width" src="/img/Fish_seafood_img/2/13.jpg" width="650px">
                    </div>
                    <div class="dish_info">
                        Подаем блюдо к столу или прямо в противне (так рыба долго не остынет).
                    </div>
                    <div class="dish_img">
                        <img class="img_width" src="/img/Fish_seafood_img/2/14.jpg" width="650px">
                    </div>
                    <div class="dish_info">
                        Или, перекладываем карасиков на отдельное блюдо! Приятного всем аппетита!
                    </div>
                </div>
            </div>
        </div>
    </main>
    <footer>
        <div class="footer_container">
            <div class="about_project, blok">
                <div class="footer_title">
                    О проекте
                </div>
                <div class="footer_info">
                    Photorecipes - кулинарный сайт с пошаговыми и поэтапными фотографиями блюд. У нас Вы найдете:
                </div>
                <ul class="footer_li">
                    <li class="footer_li_info"><a class="footer_li_info_link" href="/html/Salads/Salads.php">Рецепты различных салатов</a></li>
                    <li class="footer_li_info"><a class="footer_li_info_link" href="/html/Soups/Soups.php">Рецепты cупов</a></li>
                    <li class="footer_li_info"><a class="footer_li_info_link" href="/html/Mushrooms_vegetables/Mushrooms_vegetables.php">Различные блюда из грибов и овощей</a></li>
                    <li class="footer_li_info"><a class="footer_li_info_link" href="/html/Fish_seafood/Fish_seafood.php">Всевозможные блюда из рыбы и морепродуктов</a></li>
                    <li class="footer_li_info"><a class="footer_li_info_link" href="/html/Meat/Meat.php">Рецепты из мяса (из свинины, говядины, баранины и т.д.)</a></li>
                    <li class="footer_li_info"><a class="footer_li_info_link" href="/html/Bakery/Bakery.php">Выпечка</a></li>
                    <li class="footer_li_info"><a class="footer_li_info_link" href="/html/Desserts/Desserts.php">Закуски</a></li>
                </ul>
            </div>
            <div class="what_we_offer, blok">
                <div class="footer_title">
                    Что мы предлагаем?
                </div>
                <div class="footer_info">
                    Все блюда, размещенные на нашем
                    сайте публикуются с пошаговыми
                    фото, и представляют собой
                    наглядное пособие, которое может
                    позволить любому желающему
                    легко приготовить понравившейся
                    рецепт. Все рецепты сайта
                    возможно повторить!
                </div>
            </div>
            <div class="footer_social_network, blok">
                <div class="footer_title">
                    Мы в соцсетях
                </div>
                <div class="footer_info">
                    <div class="vk, footer_network">
                        <a href="https://vk.com/djacsal228" title="Перейти на стринуцу во Вконтакте" target="_blank"><img src="/img/VK white.png" alt="Вконтакте"></a>
                    </div>
                    <div class="facebook, footer_network">
                        <a href="https://www.facebook.com" title="Перейти на стринуцу в Фейсбуке" target="_blank"><img src="/img/Facebook white.png" alt="Фейсбук"></a>
                    </div>
                    <div class="instagram, footer_network">
                        <a href="https://www.instagram.com/viniadukhovskii" title="Перейти на стринуцу в Инстаграмм" target="_blank"><img src="/img/Instagram white.png" alt="Инстаграмм"></a>
                    </div>
                </div>
            </div>
        </div>
    </footer>
</body>
</html>